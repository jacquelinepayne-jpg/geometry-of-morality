...
sentence templates — handed to build(), which writes datasets/morality/<name>.csv, its
lexical-shortcut control datasets/morality/neg_<name>.csv, and a blind rating sheet for the
...
# this file lives in datasets/morality/, so the CSVs land beside it and the repo
# root is two levels up
DATASETS_DIR = os.path.dirname(os.path.abspath(__file__))
REPO_ROOT = os.path.dirname(os.path.dirname(DATASETS_DIR))
...
def write_dataset(dataset_name, rows, frames, seed):
    """Write datasets/morality/<name>.csv plus its rating sheet; return the dataframe.
...
def build(dataset_name, frames, names_per_frame=8, seed=0, write_negations=True):
    """Validate frames, write datasets/morality/<name>.csv and neg_<name>.csv, print stats.
