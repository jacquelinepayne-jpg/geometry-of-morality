...
python logprobs.py --model llama-2-13b --dataset human_farmed/fire_01 --device cuda:0 # statement log-probs
```

`helping_preference.py` is referenced by `documentation/` but **does not exist yet** — do
not suggest running it. `interventions.py` defaults are still the upstream truth ones
(`--train_datasets cities neg_cities`, `--val_dataset sp_en_trans`); animal/human runs
need `--train_datasets`, `--val_dataset`, `--prompt`, `--pos_token YES --neg_token NO`
set explicitly, or the few-shot prompt silently falls through to zero-shot.
...
- **Datasets** are CSVs in `datasets/` with at minimum `statement` and `label` (1/0) columns. Upstream truth datasets live in `datasets/truth/`; negated variants are prefixed `neg_`, conjunctions/disjunctions generated by `datasets/make_conj_disj.py`. New moral-valence datasets should follow this same format so the existing pipeline works unchanged.
- **Animal/human datasets are generated, not hand-edited.** `datasets/animal_human_templates.py` is the source of truth (subjects + templates + invariant assertions); `datasets/make_animal_human.py` and `datasets/make_help_continuations.py` write the CSVs. One CSV per template sentence, `cities`-style, filed in one folder per category pair: `datasets/{catA}_{catB}/{template_id}.csv`, so the **dataset name passed to `--datasets` and `DataManager` is `human_farmed/fire_01`** (nested names work throughout — `load_statements`, `collect_acts`, and the `acts/` tree all path-join it). 80 rows each, 624 for the full set. `--pilot` writes a 15-file subset; `--templates` / `--pairs` narrow it further. The 12 pooled `{catA}_{catB}/{harm|neutral}` are concatenations kept for `generalization.ipynb`; generating activations for a pooled file *and* its templates duplicates the same forward passes.
